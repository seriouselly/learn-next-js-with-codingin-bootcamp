...
const getStarwars = async (): Promise<StarwarData> => {
  const res = await axios.get<StarwarData>(`${baseUrl}/characters`);
  return res.data;
};

const StarWarsAxiosServerPage: React.FC = async () => {
  const data: StarwarData = await getStarwars();
  return (
    <>
      <h1 className="text-4xl font-bold text-gray-900 m-4">
        Star Wars Characters - Axios Server
      </h1>
...
export default StarWarsAxiosServerPage;
